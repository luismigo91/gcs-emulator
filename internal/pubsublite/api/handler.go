...
import (
	...
	"github.com/luismiguelgilolivert/gcs-emulator/internal/pubsublite/backend"
	"github.com/luismiguelgilolivert/gcs-emulator/internal/pubsublite/model"
)

type Handler struct{ Backend *backend.MemoryPubSubLiteBackend }
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) >= 5 && parts[4] == "topics" {
		...
		case tname != "" && r.Method == http.MethodGet: h.get(w, r, parent+"/topics/"+tname)
		case tname != "" && r.Method == http.MethodDelete: h.del(w, r, parent+"/topics/"+tname)
...
func (h *Handler) create(w http.ResponseWriter, r *http.Request, parent string) {
	var t model.LiteTopic; json.NewDecoder(r.Body).Decode(&t)
	if t.Name == "" { t.Name = parent + "/topics/" + r.URL.Query().Get("topicId") }
...
func (h *Handler) list(w http.ResponseWriter, r *http.Request, parent string) {
	...
	writeJSON(w, http.StatusOK, map[string]interface{}{"topics": ts})
}
